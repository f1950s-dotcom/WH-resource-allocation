...
def _adaptive_time_limit(num_int_vars: int) -> float:
    ...
    係数について：
      旧式 `5 + n/150` は、現実的な規模（整数変数 数千個）では実効上限が
      数十秒にしかならず、上限600秒はほぼ発動しなかった。そのため「重い日に
      時間切れで簡易計算へ転落する」のを十分に防げていなかった。
      そこで下限・傾きを引き上げ、実際に粘れる時間を確保する：
        例) 5,000変数 → 60 + 125 = 185秒 / 10,000変数 → 60 + 250 = 310秒。
      簡単な日はギャップ1%で早期終了するため、下限を上げても無駄に待たない。
      下限60秒〜上限600秒でクランプ（上限600 < ハード強制終了700秒）。
    """
    secs = 60.0 + num_int_vars / 40.0
    return max(60.0, min(600.0, secs))
